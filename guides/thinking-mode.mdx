...
Pass `thinking: true` in the `orchid` config object on **`/v1/chat/completions`**. Optionally set **`thinking_effort`** to `none`, `low`, `medium`, or `high`. On **[`/v1/messages`](/api-reference/messages)**, use headers **`x-orchid-thinking`** and **`x-orchid-thinking-effort`** instead — see [Models](/getting-started/models).
